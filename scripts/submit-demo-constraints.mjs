...
const sent = []
const send = async (role, account, sequence, text) => {
  ...
  sent.push({ role, transaction_hash: hash, status: 'SUBMITTED' })
...
if (!existingRoles.has('A')) await send('A', accountFor('STUDIONET_INTEGRATOR_PRIVATE_KEY'), 1, 'Keep the rollback owner named')
if (!existingRoles.has('B')) await send('B', accountFor('STUDIONET_STEWARD_PRIVATE_KEY'), 1, 'Keep incident handoff observable')
const terms = await reader.readContract({ address: deployment.contract_address, functionName: 'get_terms', args: [sessionId] })
process.stdout.write(JSON.stringify({ network: 'Studio Dev', session_id: sessionId, submitted: sent, term_count: Array.isArray(terms) ? terms.length : null }) + '\n')
