...
| `refund_expired` | sponsor | `CONFLICTING` or expired non-ratified states | ratified/refunded/reviewing | conflicting N/A; otherwise `now >= ratify_deadline` | refund false | emits exactly locked 2 GEN once | session/refund/actionability | wrong caller/state, -1/exact/+1, duplicate, no credits |
